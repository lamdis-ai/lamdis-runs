...
export default async function runsRoutes(app: FastifyInstance) {
  ...
  app.post('/internal/runs/start', async (req) => {
    ...
    const body = ((): any => {
      ...
      if (raw && Array.isArray(raw.suites)) return jsonSchema.parse(raw);
      return mongoSchema.parse(raw);
    })();

    // JSON-on-disk suites mode: run suites from local JSON files and optionally POST to webhookUrl.
    if (Array.isArray((body as any).suites)) {
      const suites = (body as any).suites as string[];
      const webhookUrl: string | undefined = (body as any).webhookUrl;
      ...
      for (const sid of suites) {
        ...
        // Re-use the same execution engine as /internal/run-file, but scoped per-suite file.
  ...
  app.post('/internal/run-file', async (req, reply) => {
    ...
    // Resolve path relative to provided cwd or process.cwd()
    ...
    if (!fs.existsSync(absPath)) {
      return reply.code(404).send({ error: 'file_not_found', filePath: absPath });
    }
    ...
    try {
      ...
      return reply.code(400).send({ error: 'file_read_or_parse_failed', message: e?.message || 'invalid_json' });
    }

    // Minimal schema: env + tests, with optional modular imports.
    const schema = z.object({
    ...
    try {
      ...
      return reply.code(400).send({ error: 'invalid_test_file', message: e?.message || 'schema_mismatch' });
    }

    if (!cfg.tests.length) {
      return reply.code(400).send({ error: 'no_tests', message: 'tests[] is empty in file' });
    }

    const orgId = cfg.orgId || 'file-org';
    const suiteId = `file-suite:${cfg.suite || path.basename(absPath)}`;

    // Resolve assistant/environment from assistantRef if provided.
    ...
    // Local, non-persisted execution – closely modeled after /internal/runs/start test loop,
    // but without Mongo/TestRun documents or usage tracking.
    const wfUrl = process.env.WORKFLOW_URL;
    const authHeader = body.authHeader || undefined;
    const judgeBase = process.env.JUDGE_BASE_URL || `http://127.0.0.1:${process.env.PORT || 3101}`;
    const judgeUrl = `${judgeBase}/orgs/${orgId}/judge`;
    ...
    return reply.send(summary as any);
  });
}
